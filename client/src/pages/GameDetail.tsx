  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[var(--background)]">
        <div className="relative h-[600px] lg:h-[700px] bg-[var(--card)] animate-pulse" />
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20">
          <div className="bg-[var(--card)] rounded-2xl p-6 mb-8">
            <div className="h-10 w-1/3 bg-[var(--muted)] rounded animate-pulse" />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <div className="h-40 bg-[var(--card)] rounded-2xl animate-pulse" />
              <div className="h-60 bg-[var(--card)] rounded-2xl animate-pulse" />
            </div>
            <div className="h-96 bg-[var(--card)] rounded-2xl animate-pulse" />
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="relative h-[600px] lg:h-[700px] overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-end h-full pb-28 lg:pb-32">
          ...
          <button
            onClick={() => navigate(-1)}
            className="absolute top-6 left-0 flex items-center gap-2 text-[var(--foreground)]/70 hover:text-[var(--foreground)] transition-colors text-sm cursor-pointer"
          >
          ...
          <div className="max-w-4xl">
            {/* Title */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-[var(--foreground)] mb-3 leading-tight">
              {game.name}
            </h1>
            ...
            {game.genres.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {game.genres.map((genre) => (
                  <span
                    key={genre}
                    className="bg-[var(--card)]/80 backdrop-blur-sm border border-[var(--border)] px-4 py-2 rounded-lg text-sm font-medium text-[var(--foreground)]"
                  >
                    {genre}
                  </span>
                ))}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-16">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {mediaItems && mediaItems.length > 0 && (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
                <h2 className="text-lg font-bold text-[var(--foreground)] mb-4">
                  <i className="fa-solid fa-images mr-2 text-[var(--text-muted)]" />
                  {t('gameDetail.media')}
                </h2>
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                        {item.type === 'video' && (
                          <div className="w-8 h-8 rounded-full bg-black/60 flex items-center justify-center">
                            <i className="fa-solid fa-play text-[var(--foreground)] text-xs" />
                          </div>
                        )}
                      </div>
                      {i === 7 && mediaItems.length > 8 && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                          <span className="text-[var(--foreground)] font-bold">+{mediaItems.length - 8}</span>
                        </div>
                      )}
                    </button>
                  ))}
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
              <h2 className="text-lg font-bold text-[var(--foreground)] mb-4">
                <i className="fa-solid fa-tag mr-2 text-[var(--text-muted)]" />
                {t('gameDetail.price')}
              </h2>
            ...
            {game.reviewScore !== null && (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
                <h2 className="text-lg font-bold text-[var(--foreground)] mb-4">
                  <i className="fa-solid fa-chart-bar mr-2 text-[var(--text-muted)]" />
                ...
                <div className="flex items-start gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[var(--foreground)] font-semibold mb-1">{reviewLabel(game.reviewScore)}</p>
                      ...
                      <div className="space-y-2">
                        <div>
                          <div className="flex items-center justify-between text-xs mb-1">
                            ...
                            <span className="text-[var(--text-muted)]">{positiveCount.toLocaleString()}</span>
                          </div>
                        ...
                        <div>
                          <div className="flex items-center justify-between text-xs mb-1">
                            ...
                            <span className="text-[var(--text-muted)]">{negativeCount.toLocaleString()}</span>
                          </div>
                          <div className="h-2 bg-[var(--muted)] rounded-full overflow-hidden">
                            <div
            ...
            {game.shortDesc && (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
                <h2 className="text-lg font-bold text-[var(--foreground)] mb-4">
                  <i className="fa-solid fa-info-circle mr-2 text-[var(--text-muted)]" />
                  {t('gameDetail.aboutThisGame')}
                </h2>
                <p className="text-[var(--text-body)] leading-relaxed">{game.shortDesc}</p>
              </div>
            )}
            ...
            {user && (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
                ...
                {reviewSummary ? (
                  <p className="text-sm text-[var(--text-body)] leading-relaxed">{reviewSummary}</p>
                ) : (
            ...
            {user && noteLoaded && (
              <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-[var(--background)] border border-[var(--border)] rounded-lg px-4 py-3 text-sm text-[var(--foreground)] placeholder-gray-500 focus:outline-none focus:border-[var(--primary)] resize-none mb-3"
                />
                <button
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 space-y-6">
              <h2 className="text-lg font-bold text-[var(--foreground)]">
                <i className="fa-solid fa-gamepad mr-2 text-[var(--text-muted)]" />
              ...
              {game.genres.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {game.genres.map((genre) => (
                      <span key={genre} className="bg-[var(--muted)] text-[var(--text-body)] px-2.5 py-1 rounded-md text-xs font-medium">
                        {genre}
                      </span>
                    ))}
                  </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-lg">
                  {game.platforms.windows && (
                    <span className="text-[var(--text-body)]" title="Windows">
                      <i className="fa-brands fa-windows" />
                    </span>
                  )}
                  {game.platforms.mac && (
                    <span className="text-[var(--text-body)]" title="macOS">
                      <i className="fa-brands fa-apple" />
                    </span>
                  )}
                  {game.platforms.linux && (
                    <span className="text-[var(--text-body)]" title="Linux">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {game.tags.slice(0, 12).map((tag) => (
                      <span key={tag} className="bg-[var(--muted)] text-[var(--text-muted)] px-2 py-0.5 rounded text-xs">
                        {tag}
                      </span>
                    ))}
                  </div>
