...
export default function Chat() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)]">
      {/* Header */}
      <div className="border-b border-[var(--border)] bg-[var(--card)]/50 px-4 sm:px-6 py-4">
        <div className="container mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)]">Chat with AI about your gaming profile</p>
          </div>
          {messages.length > 0 && (
            <button onClick={handleClear} className="text-xs text-[var(--text-muted)] hover:text-red-400 transition-colors">
        ...
        <div className="container mx-auto max-w-3xl space-y-4">
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <h2 className="text-2xl font-bold text-[var(--foreground)] mb-2">Ask me anything about your games</h2>
              <p className="text-[var(--text-muted)] mb-8">I have context of your complete gaming profile and can give personalized advice.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-lg mx-auto">
                {quickPrompts.map((prompt) => (
                  <button
                    key={prompt}
                    onClick={() => { setInput(prompt); inputRef.current?.focus(); }}
                    className="text-left px-4 py-3 bg-[var(--card)] border border-[var(--border)] hover:border-[var(--primary)] rounded-xl text-sm text-[var(--text-body)] transition-colors"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            <>
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${msg.role === 'user' ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'bg-[var(--card)] border border-[var(--border)] text-gray-200'}`}>
              ...
              {streaming && (
                <div className="flex justify-start">
                  <div className="max-w-[80%] rounded-2xl px-4 py-3 bg-[var(--card)] border border-[var(--border)] text-gray-200">
      ...
      {/* Input */}
      <div className="border-t border-[var(--border)] bg-[var(--card)]/50 px-4 sm:px-6 py-4">
        <div className="container mx-auto max-w-3xl">
          <div className="flex items-end gap-3">
            <textarea
              ...
              className="flex-1 bg-[var(--background)] border border-[var(--border)] rounded-xl px-4 py-3 text-sm text-[var(--foreground)] placeholder-gray-500 focus:outline-none focus:border-[var(--primary)] resize-none"
